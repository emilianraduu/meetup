...
export const LocationScreen = ({navigation, setCheckPerm}) => {
  ...
  const onPress = async () => {
    ...
    if (locationWhenInUse === RESULTS.GRANTED) {
      AsyncStorage.setItem('locationPerm', 'true');
      setCheckPerm?.(true);
      navigation.navigate(NotificationRoute);
    } else {
      alert(
        'Pentru a putea folosi aplicatia trebuie sa acordati permisiuni de localizare.',
      );
    }
  };
  return (
